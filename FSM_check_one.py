...
class check_payment_one(StatesGroup):
    get_hash = State()

@dp.callback_query_handler(lambda call: call.data == "check_payment_one")
async def first_step(callback_query: types.CallbackQuery, state=None):
    await check_payment_one.get_hash.set()
    await callback_query.message.edit_text("Введите хэш транзакции (TXid) строго без лишних символов:", reply_markup=back_FSM)
...
async def cancel_handler(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if current_state is None:
        return
    await state.finish()
    await callback_query.message.edit_text("Выберите подписку:", reply_markup=main_menu_keyboard)
    

@dp.message_handler(content_types=['text'], state=check_payment_one.get_hash)
async def get_hash(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if db.check_hash(hash_text=hash_text) != []:
            await bot.send_message(message.from_user.id, "❗️Данная оплата уже проходила❗️", reply_markup=back_keyboard)
            ...
            if float(amount) == float(get_amount)/10**6 and ADDRESS == str(my_address):
                ...
                await bot.send_message(message.from_user.id, text="Произошла ошибка при проверке оплаты, обратитесь в поддержку", reply_markup=back_keyboard)
    await state.finish()
       
       
def register_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(first_step, lambda call: call.data == "check_payment_one", state=None)
    dp.register_callback_query_handler(cancel_handler, lambda call: call.data == "back", state="*")
    dp.register_message_handler(get_hash, state=check_payment_one.get_hash)
